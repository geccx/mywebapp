...
export default function Dashboard() {
  ...
  const loadItems = async () => {
    try {
      ...
      if (Array.isArray(res.data)) setItems(res.data);
        ...
        console.error("GET /items returned non-array:", res.data);
        setItems([]);
      }
    } catch (err) {
  ...
  return (
    <>
      <Navbar onProfile={() => setOpenProfile(true)} onLogout={logout} />
      ...
      >
        <div>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
            ...
            <button className="btn-muted" onClick={() => setOpenEdit(false)}>
              Cancel
            </button>
          </div>
        </div>
      ...
      >
        <div>
          ...
          <div style={{ marginTop: 8 }}>
            ...
            <input
              type="file"
              onChange={(e) => setPfImage(e.target.files[0])}
            />
          </div>

          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            <button onClick={submitProfile}>Save Profile</button>
            <button className="btn-muted" onClick={() => setOpenProfile(false)}>
              Cancel
            </button>
          </div>
        </div>
